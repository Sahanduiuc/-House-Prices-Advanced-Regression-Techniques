def model(X,y,z):
    ...
    par_rf = {'n_estimators': [100, 150, 200],
              'max_depth' : [3,6,9]}
    par_dt = {'max_features': ['auto', 'sqrt', 'log2'],
              'max_depth' : [3,6,9]}
    par_xg = {'n_estimators': [100, 150, 200],
              'max_depth' : [3,6,9]}
    
    model1 = RandomizedSearchCV(estimator=RandomForestRegressor(random_state=42), param_distributions=par_rf, cv= 5, n_iter=1,scoring=my_scorer)
    model2 = RandomizedSearchCV(estimator=DecisionTreeRegressor(random_state=42), param_distributions=par_dt, cv= 5, n_iter=1,scoring=my_scorer)
    model3 = RandomizedSearchCV(estimator=xgb.XGBRegressor(random_state=42), param_distributions=par_xg, cv= 5, n_iter=1,scoring=my_scorer)
